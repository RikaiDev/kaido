...
"#;
    ...
    assert!(
        explanation.is_some(),
        "Drush should provide error explanation"
    );
    ...
    assert_eq!(
        exp.recommended_solution, 0,
        "Should recommend first solution"
    );
...
fn test_drush_correct_commands() {
    ...
    let correct_commands = vec!["drush cr", "drush sql:cli", "drush sqlq 'SHOW TABLES'"];
    ...
    for cmd in correct_commands {
        ...
        assert!(
            explanation.is_none(),
            "Should not explain error for correct command: {cmd}"
        );
        println!("✓ '{cmd}' - recognized as correct");
    }
}
...
fn test_complete_drush_workflow() {
    ...
    assert!(
        solution
            ...
            .as_ref()
            .unwrap()
            .contains("sql:cli < database.mysql"),
