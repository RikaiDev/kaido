...
use kaido::tools::{RiskLevel, ToolContext, ToolRegistry};
...
fn test_risk_confirmation_requirements() {
    ...
    assert!(
        RiskLevel::High.requires_typed_confirmation(true),
        "HIGH in production needs typed"
    );
    assert!(
        !RiskLevel::High.requires_typed_confirmation(false),
        "HIGH in dev doesn't need typed"
    );
    assert!(
        RiskLevel::Critical.requires_typed_confirmation(true),
        "CRITICAL always needs typed"
    );
    assert!(
        RiskLevel::Critical.requires_typed_confirmation(false),
        "CRITICAL always needs typed"
    );
}
