...
fn test_detect_kubectl_commands() {
    ...
    for (input, expected) in test_cases {
        ...
        if let Some(t) = tool {
            ...
            println!("✗ '{}' → no tool detected", input);
...
fn test_ambiguous_input_detection() {
    ...
    for input in test_cases {
        ...
        match tool {
            ...
            None => println!("✗ '{}' → no tool detected (expected, too ambiguous)", input),
        }
    }
}
...
fn test_explicit_tool_command_priority() {
    ...
    for (input, expected) in explicit_commands {
        let tool = registry.detect_tool(input).expect("Should detect explicit command");
