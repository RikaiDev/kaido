...
use kaido::tools::{RiskLevel, ToolRegistry};
...
fn showcase_kubernetes_production_safety() {
    ...
    let dangerous_commands = vec![
        (
            ...
            "CRITICAL - deleting entire namespace",
        ),
        (
            "kubectl delete pods --all -n prod",
            "CRITICAL - batch deletion",
        ),
        (
            "kubectl drain node-prod-01 --force",
            "HIGH - forcing node drain",
        ),
        (
            "kubectl scale deployment api --replicas=0",
            "HIGH - scaling to zero",
        ),
    ];
...
fn showcase_docker_container_management() {
    ...
    for (cmd, expected_risk, description) in operations {
        ...
        let match_icon = if actual_risk == expected_risk {
            "✓"
        } else {
            "✗"
        };
...
fn showcase_error_explanation_kubectl() {
    ...
    let explanation = matcher
        .match_pattern(error)
        .expect("Should match kubectl RBAC error");
...
fn showcase_error_explanation_docker() {
    ...
    let explanation = matcher
        .match_pattern(error)
        .expect("Should match Docker daemon error");
...
fn showcase_tool_detection_ambiguous() {
    ...
    let test_cases = vec![
        ...
        (
            "get pods in namespace prod",
            None,
            "Has 'pods' keyword but < 50% threshold",
        ),
    ];
    ...
    for (input, expected, description) in test_cases {
        ...
        let icon = if detected_name == expected {
            "✓"
        } else {
            "✗"
        };

        println!("{icon} Input: \"{input}\"");
        match detected_name {
