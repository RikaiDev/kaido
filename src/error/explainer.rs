use anyhow::Result;
use crate::tools::{ErrorExplanation, ToolContext, LLMBackend};
...
"#,
    ...
    fn parse_llm_explanation(&self, llm_response: &str) -> Result<ErrorExplanation> {
        ...
        let error_type = parsed["error_type"].as_str()
            .unwrap_or("Unknown Error")
            .to_string();
        
        let reason = parsed["reason"].as_str()
            .unwrap_or("無法解釋此錯誤")
            .to_string();
        ...
        let solutions: Vec<crate::tools::Solution> = parsed["solutions"]
            .as_array()
            .map(|arr| arr.iter().filter_map(|sol| {
        ...
        let recommended_solution = parsed["recommended_solution"].as_u64()
...
mod tests {
    ...
    fn test_error_explainer_creation() {
        ...
        assert!(explainer.pattern_matcher.match_pattern("kubectl error").is_none());
    }
}
