...
impl RootCauseAnalyzer {
    ...
    pub fn get_applicable_strategies(&self, problem: &ProblemContext) -> Vec<&dyn DiagnosisStrategy> {
        self.strategies
            .iter()
            ...
            if source.contains("netstat") || source.contains("ss") || source.contains("lsof") {
                ...
                if data.contains(":80 ") || data.contains(":443 ") || data.contains(":8080 ") {
                    return Some(RootCause {
                        ...
                        evidence: vec![format!("Found from {}: {}", source, data.lines().take(3).collect::<Vec<_>>().join("; "))],
                        confidence: 85,
                        affected_components: vec![problem.service.clone().unwrap_or_else(|| "unknown".to_string())],
                    });
                }
            }
        }
    ...
    fn diagnostic_commands(&self, problem: &ProblemContext) -> Vec<DiagnosticCommand> {
        ...
        if let Some(service) = &problem.service {
            commands.push(DiagnosticCommand::new(
                ...
                &format!("Check {} status", service)
            ));
            
            commands.push(DiagnosticCommand::new(
                "journalctl",
                &format!("journalctl -u {} -n 50 --no-pager", service),
                &format!("Check {} recent logs", service)
            ));
        }
    ...
    fn analyze(&self, problem: &ProblemContext) -> Option<RootCause> {
        for (source, data) in &problem.diagnostic_data {
            if source.contains("systemctl status") {
                if data.contains("inactive") || data.contains("failed") {
                    return Some(RootCause {
                        category: RootCauseCategory::ServiceDown,
                        description: "Service is not running or has failed".to_string(),
                        evidence: vec![format!("systemctl status shows: {}", data.lines().take(2).collect::<Vec<_>>().join(" "))],
                        confidence: 90,
                        affected_components: vec![problem.service.clone().unwrap_or_else(|| "unknown".to_string())],
                    });
                }
        ...
        for (source, data) in &problem.diagnostic_data {
                ...
                || data.contains("configuration test failed")
            {
                return Some(RootCause {
                    ...
                    evidence: vec![format!("From {}: {}", source, data.lines().take(3).collect::<Vec<_>>().join("; "))],
                    confidence: 95,
                    affected_components: vec![problem.service.clone().unwrap_or_else(|| "unknown".to_string())],
                });
            }
