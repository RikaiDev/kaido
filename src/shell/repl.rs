...
impl KaidoREPL {
    ...
    async fn run_agent(&mut self, problem: &str) -> Result<()> {
        ...
        println!(
            "\x1b[38;5;245m│\x1b[0m status:   \x1b[38;5;147m{:?}\x1b[0m",
            final_state.status
        );
        println!(
            "\x1b[38;5;245m│\x1b[0m steps:    {}",
            final_state.history.len()
        );
        println!(
            "\x1b[38;5;245m│\x1b[0m actions:  {}",
            final_state
                .history
                .iter()
        ...
        println!(
            "\x1b[38;5;245m│\x1b[0m duration: {:.2}s",
            final_state.start_time.elapsed().as_secs_f64()
        );
        ...
        if let Some(solution_plan) = &final_state.solution_plan {
            ...
            for (i, step) in solution_plan.iter().enumerate() {
                println!(
                    "\x1b[38;5;245m│\x1b[0m   \x1b[38;5;242m{}.\x1b[0m {}",
                    i + 1,
                    step
                );
            }
    ...
    fn display_step_static(step: &AgentStep) {
        match step.step_type {
            ...
            StepType::Observation => {
                ...
                println!("{color}{status}\x1b[0m");

                println!(
                    "\n\x1b[38;5;250m╭─ OBSERVATION #{}\x1b[0m",
                    step.step_number
                );
                let content = if step.content.len() > 600 {
                    format!(
                        "{}... \x1b[38;5;237m(truncated)\x1b[0m",
                        &step.content[..600]
                    )
                } else {
                    step.content.clone()
    ...
    fn print_help(&self) {
        ...
        println!(
            "\x1b[38;5;245m│\x1b[0m   \x1b[38;5;242m4.\x1b[0m Reflection  → Evaluate progress"
        );
