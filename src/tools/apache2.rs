...
impl Apache2Tool {
    ...
    pub async fn validate_config() -> Result<String> {
        ...
        let output = tokio::process::Command::new(if cmd == "apache2" {
            "apache2ctl"
        } else {
            "apachectl"
        })
...
impl Tool for Apache2Tool {
    ...
    async fn translate(
        ...
        let parsed: ApacheResponse =
            serde_json::from_str(&llm_response.reasoning).unwrap_or(ApacheResponse {
            ...
            return Some(ErrorExplanation {
                ...
                solutions: vec![
                    ...
                    Solution {
                        ...
                        command: Some(
                            "systemctl stop apache2 && ps aux | grep apache2".to_string(),
                        ),
...
mod tests {
    ...
    fn test_classify_risk() {
        ...
        assert_eq!(
            tool.classify_risk("apache2ctl configtest", &ctx),
            RiskLevel::Low
        );
        assert_eq!(
            tool.classify_risk("systemctl restart apache2", &ctx),
            RiskLevel::Medium
        );
        assert_eq!(
            tool.classify_risk("systemctl stop apache2", &ctx),
            RiskLevel::High
        );
    }
}
