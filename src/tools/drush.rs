...
impl DrushTool {
    pub fn new() -> Self {
        Self {
            drush_path: which::which("drush").unwrap_or_else(|_| PathBuf::from("vendor/bin/drush")),
        }
    }
...
"#,
    ...
    fn explain_error(&self, error: &str) -> Option<ErrorExplanation> {
        ...
        if error.contains("ERROR 1064") && (error.contains(".mysql") || error.contains(".sql")) {
            // Extract filename
            let filename = extract_filename_from_error(error).unwrap_or("database.mysql");
            ...
            return Some(ErrorExplanation {
                ...
                solutions: vec![
                    ...
                    Solution {
                        ...
                        command: Some(format!("cat {filename} | vendor/bin/drush sqlq")),
                        risk_level: RiskLevel::Medium,
                    },
                    Solution {
...
mod tests {
    ...
    fn test_extract_filename() {
        ...
        assert_eq!(extract_filename_from_error("drush cr"), None);
    }
