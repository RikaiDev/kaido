...
impl NginxTool {
    ...
    pub async fn check_port_usage(port: u16) -> Result<String> {
        let output = tokio::process::Command::new("sh")
            .arg("-c")
            .arg(format!("lsof -i :{port} -P -n || ss -tlnp | grep :{port}"))
            .output()
            .await?;
...
impl Tool for NginxTool {
    ...
    async fn translate(
        ...
        let parsed: NginxResponse =
            serde_json::from_str(&llm_response.reasoning).unwrap_or(NginxResponse {
            ...
            return Some(ErrorExplanation {
                ...
                solutions: vec![
                    ...
                    Solution {
                        ...
                        command: Some(
                            "# chmod 644 /path/to/file && chown nginx:nginx /path/to/file"
                                .to_string(),
                        ),
                        risk_level: RiskLevel::Medium,
                    },
...
mod tests {
    ...
    fn test_classify_risk() {
        ...
        assert_eq!(
            tool.classify_risk("nginx -s reload", &ctx),
            RiskLevel::Medium
        );
        assert_eq!(
            tool.classify_risk("systemctl stop nginx", &ctx),
            RiskLevel::High
        );
        assert_eq!(
            tool.classify_risk("apt remove nginx", &ctx),
            RiskLevel::Critical
        );
    }
