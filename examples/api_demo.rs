...
async fn main() -> anyhow::Result<()> {
    ...
    match ai_manager.infer(test_prompt).await {
        Ok(response) => {
            ...
            println!(
                "    Reasoning: {}",
                response.reasoning.lines().next().unwrap_or("")
            );
        }
        Err(e) => {
    ...
    match command_engine.classify_risk(&translation, &context) {
        Ok(risk) => {
            ...
            println!(
                "   Requires Typed Confirmation (prod): {}",
                risk.requires_typed_confirmation(true)
            );
            println!(
                "   Requires Typed Confirmation (dev): {}",
                risk.requires_typed_confirmation(false)
            );
        }
        Err(e) => {
    ...
    println!(
        "    Is Low Confidence: {}",
        kubectl_translation.is_low_confidence(90)
    );
